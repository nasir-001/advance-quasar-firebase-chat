<template>
  <div>
    ...
    >
      ...
      <select v-else v-model="currentUserId">
        <option v-for="user in liveUsers" :key="user._id" :value="user._id">
          {{ user.username }}
        </option>
      </select>
...
export default {
  ...
  async mounted() {
    ...
    this.testUsers = await firestoreService.getAllUsers();
    this.liveUsers = this.testUsers.data;
